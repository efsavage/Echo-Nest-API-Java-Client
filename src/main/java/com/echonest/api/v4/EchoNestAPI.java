...
public class EchoNestAPI {
    ...
    @SuppressWarnings("unchecked")
    public Track uploadTrack(File audioFile, boolean wait)
     ...
     * 
     * @param p
     *            the playlist params (see PlaylistParams)
     * @return the playlist
     * @throws EchoNestException
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Creates a dynamic playlist. Returns a dynamic playlist. A dynamic
     * playlist is created with an initial set of parameters and songs are
     * fetched one at a time using a session identifier. The playlist is dynamic
     * and will return songs based on the listener's feedback. A dynamic
     * playlist returns a single song at a time along with the session ID. When
     * the client is ready to play the next song it makes a dynamic playlist
     * request that includes the session ID
     * 
     * @param p
     *            the params for the playlist (see PlaylistParams)
     * @return the playlist
     * @throws EchoNestException
     */
    @SuppressWarnings("unchecked")
    public Playlist createDynamicPlaylist(Params p) throws EchoNestException {
        List<Song> songResults = new ArrayList<Song>();
        Map results = cmd.sendCommand("playlist/dynamic", p);
        Map response = (Map) results.get("response");
        ...
        Playlist playlist = new Playlist(songResults, session);
        return playlist;
    }
     ...
     *            the session id
     * @param rating
     *            a rating of 1 to 5 (zero means no rating)
     * @return the playlist
     * @throws EchoNestException
     */
    public Playlist getNextInDynamicPlaylist(String sessionID, int rating)
            throws EchoNestException {
        return getNextInDynamicPlaylist(sessionID, rating, 0);
    }

    @SuppressWarnings("unchecked")
    public Playlist getNextInDynamicPlaylist(String sessionID, int rating,
            int testMode) throws EchoNestException {
        Params p = new Params();
     ...
     * @throws EchoNestException
     */
    public Playlist getNextInDynamicPlaylist(String sessionID)
            throws EchoNestException {
        return getNextInDynamicPlaylist(sessionID, 0);
    }
